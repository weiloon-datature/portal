...
def main(args):
    ...
    ), "model_input_shape should be multiples of 32"

    # get different model type & train&val data generator
    if args.model_type.startswith(
        "scaled_yolo4_"
    ) or args.model_type.startswith("yolo5_"):
        ...
        "tiny_yolo3_"
    ) or args.model_type.startswith("tiny_yolo4_"):
        # elif num_anchors == 6:
    ...
    pruning_end_step = (
        np.ceil(1.0 * num_train / args.batch_size).astype(np.int32)
        * args.total_epoch
    )
